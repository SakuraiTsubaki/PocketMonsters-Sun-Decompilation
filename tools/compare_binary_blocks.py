...
def main()->int:
    ...
    ap.add_argument("-o","--output",type=Path,default=Path("binary_block_diff.json"))
    ap.add_argument("--block-size",type=lambda x:int(x,0),default=0x1000)
    a=ap.parse_args()
    if a.block_size<=0: ap.error("block size must be > 0")
    l=a.left.read_bytes(); r=a.right.read_bytes(); n=max(len(l),len(r)); rows=[]
    same=changed=left_only=right_only=0
    ...
    print(f"compared {len(rows)} blocks -> {a.output}"); return 0
